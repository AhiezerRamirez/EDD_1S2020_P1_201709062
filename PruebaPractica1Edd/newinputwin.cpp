...
newinputwin::newinputwin(WINDOW *win, int y, int x,ListaDoble *listaD,ListaSimpleOrdenada *listao,ListaSimpleOrdenada *listao2,Pila *undo,Pila *redo,Estructura *estructura,ListaCircular *listarutas){
    textwin=win;
    this->locy=y;
    this->locx=x;
    this->listaD=listaD;
...
void newinputwin::typing(char caracter){
    //newinputwin::boublelist.insertarFinal(caracter,false);
    mvwaddch(textwin,locy,locx,caracter);
}

void newinputwin::reemplazar(std::string palabra,std::string nuevapalabra){
    //std::string palabra;
    //palabra="van";
...
std::vector<std::string> newinputwin::split(std::string strToSplit, char delimeter)
{
